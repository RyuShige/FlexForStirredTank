...
class StirredTank3D : public Scene
{
public:

	StirredTank3D(const char* name) : Scene(name) {}

	void Initialize()
	{
		const float radius = 0.0049625f * 3.f;
		const float restDistance = radius*0.6f;

		float center_y = 0.23f / 2.f;
		Vec3 center = Vec3(0, center_y, 0);
		float scale = 0.3f; //実実験の深さ(0.15)/blenderでの作成サイズ(0.5)
		
		Quat rot = QuatFromAxisAngle(Vec3(0.0f, 1.0f, 0.0f), M_PI / 2.f); //部品を回転させたいときに使う

		Mesh* paddle = ImportMesh("../../data/paddle_3D_keisya_2.obj");
		...
		AddTriangleMesh(mesh, Vec3(0.f, 0.15f * 0.4f, 0.f), Quat(), Vec3(1.f));
		mesh1 = CreateTriangleMesh(container);
		AddTriangleMesh(mesh1, Vec3(center), Quat(), Vec3(1.f));

		float thickness = 0.25f / 10.f;
		...
		//流体充填
		float asobi = 0.075f * 0.5f;
		float min_xz = -0.075f + asobi;
		float max_xz = 0.075f - asobi;
		float min_y = 0.f;
		float max_y = 0.15f + asobi * 2.f;
		random_device rd;
	...
	float rotation;
	float rotationSpeed;
	//このシーン特有の変数,NvFlexExtMovingFrameApply()に加えている
