...
//そのタイムステップの最大速度..drawArrowと合わせると二重ループ、、それ以前に上手くいってない
//float maxSpeed() {
//	float max = abs(g_buffers->velocities[0].x) + abs(g_buffers->velocities[0].z);
//	for (int i = 1; i < 10000; i++) {
//		if (max < (abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z)))
//			max = abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z);
//	}
//	return max;
//}

//閾値以下を0で返す関数
float speed(float minSpeed, float v) {
	if (v < minSpeed)
		return 0;
	else
		return (v - minSpeed);
...
//速度グラデーション
Vec4 speedGradation(float maxSpeed, float minSpeed, float v) {
	float halfSpeed = (maxSpeed - minSpeed) / 2.f;
	if (v < halfSpeed) {
		//return Vec4((255.f - 0.f)*(v / (maxSpeed / 2.f)) + 0.f,
		//	        (255.f - 0.f)*(v / (maxSpeed / 2.f)) + 0.f,
		//	        (255.f - 255.f)*(v / (maxSpeed / 2.f)) + 255.f,
		//	         0.f);
		return Vec4(1.f*speed(minSpeed, v) / halfSpeed + 0.f,
			        1.f*speed(minSpeed, v) / halfSpeed + 0.f,
			        1.f,
			        0.f);
	}
	else {
		return Vec4(1.f,
			        1.f - 1.f * (v - halfSpeed) / (maxSpeed - minSpeed),
			        1.f - 1.f * (v - halfSpeed) / (maxSpeed - minSpeed),
			        0.f);
	}
	//return Vec4(1.f,
...
void drawArrow(float scale) {
	for (int i = 0; i < (g_buffers->positions.size()); ++i) {
		...
		Vec4 gradation = speedGradation(0.199f, 0.f, abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z));

		Vec3 startPoint = g_buffers->positions[i];
		float startPointX = g_buffers->positions[i].x;
		float startPointY = g_buffers->positions[i].y;
		float startPointZ = g_buffers->positions[i].z;

		Vec3 endPoint = g_buffers->positions[i] + (
			        (
			        	g_buffers->velocities[i].x * Vec4(1, 0, 0, 0) +
			        	g_buffers->velocities[i].z	* Vec4(0, 0, 1, 0)
			        	) /
			         sqrt(pow(g_buffers->velocities[i].x, 2) + pow(g_buffers->velocities[i].z, 2))
			         )*scale;
		float endPointX = endPoint.x;
		float endPointY = endPoint.y;
		float endPointZ = endPoint.z;

		//Vec3 endPointWing = g_buffers->positions[i] + (
		//	        (
		//	        	g_buffers->velocities[i].x * Vec4(1, 0, 0, 0) +
		//	        	g_buffers->velocities[i].z	* Vec4(0, 0, 1, 0)
		//	        	) /
		...
		DrawLine(startPoint,
			     ...
			     gradation);
		//羽
		float rArrowHead = atan2(endPointZ - startPointZ, endPointX - startPointX);
		
		float arrowHeadLX = -scale/2.f/2.f;
		float arrowHeadLZ = scale/4.f/2.f;
		float arrowHeadRX = -scale/2.f/2.f;
		float arrowHeadRZ = -scale/4.f/2.f;
		
		float newArrowHeadLX = arrowHeadLX*cos(rArrowHead) - arrowHeadLZ*sin(rArrowHead);
		float newArrowHeadLZ = arrowHeadLX*sin(rArrowHead) + arrowHeadLZ*cos(rArrowHead);
		float newArrowHeadRX = arrowHeadRX*cos(rArrowHead) - arrowHeadRZ*sin(rArrowHead);
		float newArrowHeadRZ = arrowHeadRX*sin(rArrowHead) + arrowHeadRZ*cos(rArrowHead);
		//Left
		DrawLine(endPoint, 
			     Vec3(newArrowHeadLX + endPointX, 
				      startPointY, 
				      newArrowHeadLZ + endPointZ), 
			          gradation);
		//Right
		DrawLine(endPoint,
			Vec3(newArrowHeadRX + endPointX,
				startPointY,
				newArrowHeadRZ + endPointZ),
			    gradation);
	}
}

void drawArrow3D(float scale, float diameter, float threshold, int iter) {
	for (int i = 0; i < iter; ++i) {
		if (g_buffers->positions[i].z < diameter * threshold && g_buffers->positions[i].z >  -diameter * threshold) {

			Vec4 gradation = speedGradation(0.199f * 0.5f, 0.f, abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].y));
			...
			Vec3 endPoint = g_buffers->positions[i] + (
				(
					g_buffers->velocities[i].x * Vec4(1, 0, 0, 0) +
					g_buffers->velocities[i].y	* Vec4(0, 1, 0, 0)
					) /
				sqrt(pow(g_buffers->velocities[i].x, 2) + pow(g_buffers->velocities[i].y, 2))
				)*scale;
			float endPointX = endPoint.x;
			float endPointY = endPoint.y;
			float endPointZ = endPoint.z;
			...
			DrawLine(startPoint,
				...
				gradation);
			//羽
			float rArrowHead = atan2(endPointY - startPointY, endPointX - startPointX);

			float arrowHeadLX = -scale / 2.f / 2.f;
			float arrowHeadLY = scale / 4.f / 2.f;
			float arrowHeadRX = -scale / 2.f / 2.f;
			float arrowHeadRY = -scale / 4.f / 2.f;

			float newArrowHeadLX = arrowHeadLX*cos(rArrowHead) - arrowHeadLY*sin(rArrowHead);
			float newArrowHeadLY = arrowHeadLX*sin(rArrowHead) + arrowHeadLY*cos(rArrowHead);
			float newArrowHeadRX = arrowHeadRX*cos(rArrowHead) - arrowHeadRY*sin(rArrowHead);
			float newArrowHeadRY = arrowHeadRX*sin(rArrowHead) + arrowHeadRY*cos(rArrowHead);
			//Left
			DrawLine(endPoint,
				Vec3(newArrowHeadLX + endPointX,
					newArrowHeadLY + endPointY,
					startPointZ),
				gradation);
			//Right
			DrawLine(endPoint,
				Vec3(newArrowHeadRX + endPointX,
					newArrowHeadRY + endPointY,
					startPointZ),
				gradation);
		}
	}
...
void drawArrow3DXYZ(float scale, float diameter, float thresholdPercentXZ, float thresholdPercentY, int iter, float thresholdV) {
	for (int i = 0; i < iter; ++i) {

		float velocity = abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].y) + abs(g_buffers->velocities[i].z);

		if (pow(g_buffers->positions[i].x, 2) + pow(g_buffers->positions[i].z, 2) < pow(diameter * thresholdPercentXZ / 2.f, 2) && pow(g_buffers->positions[i].x, 2) + pow(g_buffers->positions[i].z, 2) >  -pow(diameter * thresholdPercentXZ / 2.f, 2) &&
			g_buffers->positions[i].y < (diameter * thresholdPercentY / 2.f + 0.25f) && g_buffers->positions[i].y >  (-diameter * thresholdPercentY / 2.f + 0.25f) &&
			velocity > 0.199f * thresholdV) {

			Vec4 gradation = speedGradation(0.199f, 0.f, velocity);
			...
			Vec3 endPoint = g_buffers->positions[i] + (
				...
				sqrt(pow(g_buffers->velocities[i].x, 2) + pow(g_buffers->velocities[i].y, 2) + pow(g_buffers->velocities[i].z, 2))
				)*scale;
			float endPointX = endPoint.x;
			float endPointY = endPoint.y;
			float endPointZ = endPoint.z;
			...
			DrawLine(startPoint,
				...
				gradation);
			//羽
			float rArrowHeadYX = atan2(endPointY - startPointY, endPointX - startPointX);	//回転角	
			float rArrowHeadZX = atan2(endPointZ - startPointZ, endPointX - startPointX);	//Zの情報
			...
			float newArrowHeadLX = arrowHeadLX*cos(rArrowHeadYX) - arrowHeadLY*sin(rArrowHeadYX);
			float newArrowHeadLY = arrowHeadLX*sin(rArrowHeadYX) + arrowHeadLY*cos(rArrowHeadYX);
			float newArrowHeadRX = arrowHeadRX*cos(rArrowHeadYX) - arrowHeadRY*sin(rArrowHeadYX);
			float newArrowHeadRY = arrowHeadRX*sin(rArrowHeadYX) + arrowHeadRY*cos(rArrowHeadYX);

			float newArrowHeadLZ = arrowHeadLX*sin(rArrowHeadZX) + arrowHeadLY*cos(rArrowHeadZX);
			float newArrowHeadRZ = arrowHeadRX*sin(rArrowHeadZX) + arrowHeadRY*cos(rArrowHeadZX);
			//Left
			DrawLine(endPoint,
				Vec3(newArrowHeadLX + endPointX,
					newArrowHeadLY + endPointY,
...
void drawRyumyakusen() {
	for (int i = g_frame - 2000; i < g_frame; i++) {
		Vec4 gradation = speedGradation(0.199f * 1.125f, 0.f, stockVelocities[i]);

		Vec3 startPoint = stockPositions[i - 1];
	
		Vec3 endPoint = stockPositions[i];
...
void drawRyumyakusen3DXYZ(float scale, float diameter, float thresholdPercentXZ, float thresholdPercentY, int iter, float thresholdV) {
	for (int i = 0; i < iter; ++i) {
		...
		if (pow(g_buffers->positions[i].x, 2) + pow(g_buffers->positions[i].z, 2) < pow(diameter * thresholdPercentXZ / 2.f, 2) && pow(g_buffers->positions[i].x, 2) + pow(g_buffers->positions[i].z, 2) >  -pow(diameter * thresholdPercentXZ / 2.f, 2) &&
			...
			Vec3 startPoint = g_buffers->positions[i - 1];
			float startPointX = g_buffers->positions[i].x;
			float startPointY = g_buffers->positions[i].y;
			float startPointZ = g_buffers->positions[i].z;
...
//.csv出力
void fileOutputSpeed() {
	const char *filename = "/.csv"; //自動で名前変わるようにしたい

	ofstream ofs(filename, ios::app);//追加書き込み
	//ofstream ofs(filename);//更新書き込み

	for (int i = 0; i < 6528; i++) {
		ofs << g_frame << "," << i << "," << abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z) << endl;
	}
	cout << filename << "に書き込みました" << endl;
}

void fileOutputSpeedAt() {
	const char *filename = "/.csv"; //自動で名前変わるようにしたい
	...
	for (int i = 0; i < 6528; i++) {
		if (round(g_buffers->positions[i].x * 1000.f) == round(g_buffers->positions[i].z * 1000.f)) {
			ofs << g_frame << "," << i << "," << g_buffers->positions[i].x << "," << g_buffers->positions[i].z << "," << abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z) << endl;
		}
	}
	cout << filename << "に書き込みました" << endl;
}

void fileOutputSpeedSameIntervals() {
	const char *filename = "/.csv"; //自動で名前変わるようにしたい
	...
	for (int i = 0; i < 6528; i++) {
		if (round(g_buffers->positions[i].x * 1000.f) == round(g_buffers->positions[i].z * 1000.f) && fmod(round(abs(g_buffers->positions[i].x) * 1000), 14) == 0) {
			ofs << g_frame << "," << i << "," << g_buffers->positions[i].x << "," << g_buffers->positions[i].z << "," << abs(g_buffers->velocities[i].x) + abs(g_buffers->velocities[i].z) << endl;
		}
	}
	cout << filename << "に書き込みました" << endl;
