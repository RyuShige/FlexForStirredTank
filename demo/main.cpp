...
int numRenderPar; //レンダリングする流体粒子の数

bool g_drawOpaque;
int g_drawSprings;		// 0: no draw, 1: draw stretch 2: draw tether
bool g_drawArrow2D = false;
bool g_drawContacts = false;
bool g_drawRyumyakusen = false;
bool g_drawDiffuse;
...
Vec4 stockPositions[10000000];
float stockVelocities[10000000];
...
struct Emitter
{
	Emitter() : mSpeed(0.0f), mEnabled(false), mLeftOver(0.0f), mWidth(5)   {}
...
void Init(int scene, bool centerCamera = true)
{
	...
	g_params.maxSpeed = FLT_MAX;//float の最大の有限値を表すマクロ
	g_params.maxAcceleration = 100.0f;	// approximately 10x gravity

	g_params.relaxationMode = eNvFlexRelaxationLocal;//保留
	g_params.relaxationFactor = 1.0f;//保留
...
void UpdateEmitters()
{
	...
	if (g_emit)
	{
		...
		for (; e < g_emitters.size(); ++e)
		{
			...
			//g_buffers->positions[4461] = g_buffers->positions[0];
			//for (int l = 0; l < n; ++l) {
			//	g_buffers->positions[l] = g_buffers->positions[n - l];
			//	//g_buffers->velocities[l] = g_buffers->velocities[n - l];
			//	//g_buffers->phases[l] = g_buffers->phases[n - l];
			//	//g_buffers->activeIndices.push_back(activeCount);
			//}
		}
	}
...
void UpdateMouse()
{
	...
	if (g_mousePicked)
	{
		...
		if (g_mouseParticle != -1)
		{
			...
			printf("picked: %d, mass: %f p: %f %f %f\n", g_mouseParticle, g_buffers->positions[g_mouseParticle].w, g_buffers->positions[g_mouseParticle].x, 
...
void UpdateScene()
{
	...
	// suldgeの値だけ取ればよい
	//for (int i = 0; i < g_buffers->positions.size(); i++) {
	//	printf("%d, %d, %f, %f, %f\n", g_frame, i, g_buffers->velocities[i].x,
	//		                                       g_buffers->velocities[i].y,
	//		                                       g_buffers->velocities[i].z);
	//}
	...
	//		Vec3(g_buffers->positions[i] + g_buffers->velocities[i].x/*+g_buffers->velocities[i].z*/),
	//		Vec4(0.0f, 1.0f, 0.0f, 0.0f));
	//}
	//
	//EndLines();
	int number = 29;
	stockPositions[g_frame] = g_buffers->positions[number];
	stockVelocities[g_frame] = abs(g_buffers->velocities[number].x) + abs(g_buffers->velocities[number].y) + abs(g_buffers->velocities[number].z);
}
...
void RenderScene()
{
	...
	if (g_drawEllipsoids)
	{
		...
		RenderEllipsoids(g_fluidRenderer, g_fluidRenderBuffers, numRenderPar, g_numSolidParticles, radius, float(g_screenWidth), aspect, fov, g_lightPos, g_lightTarget, lightTransform, g_shadowMap, g_fluidColor, g_blur, g_ior, g_drawOpaque);
...
void RenderDebug()
{
	...
	if (g_drawArrow2D)
	{
		...
		//2D
		float scaleArrow = 0.0018125f;

		drawArrow(scaleArrow);

		EndLines();
	}

	if (g_drawRyumyakusen)
	{
		BeginLines();

		//3D
		float scaleArrow = 0.015f * 0.375f;

		//drawArrow3D(scaleArrow, 0.5f, 0.0125f);
		//drawArrow3DXYZ(scaleArrow, 0.5f, 0.5f, 0.5f, 5000, 0.7f);	//Re=8, 局所
		//drawArrow3DXYZ(scaleArrow, 0.5f, 1.f, 0.25f, 2500, 0.f);	//Re=8, 全体
		//drawArrow3D(scaleArrow, 0.15f, 0.05f, 2500);	//
		drawRyumyakusen();	//Re=8, 全体
	...
	else if (type == eNvFlexShapeSDF)
	{
		if (g_fields.find(geo.sdf.field) != g_fields.end())
		{
			...
			if (m)
			{
				...
				DrawGpuMesh(m, xform, Vec3(color));//コメントアウトでmeshのレンダリングオフ
			}
		}
	}
...
int DoUI()
{
	...
	if (g_showHelp)
	{
		...
		if (g_tweakPanel)
		{
			...
			if (imguiCheck("Draw Arrow2D", g_drawArrow2D))
				g_drawArrow2D = !g_drawArrow2D;
			...
			if (imguiCheck("Draw Ryumyakusen", g_drawRyumyakusen))
				g_drawRyumyakusen = !g_drawRyumyakusen;
		...
		if (imguiButton("Reset Scene"))
			...
			if (imguiSlider("Num Iterations", &n, 1, 160, 1))
			...
			imguiSlider("SOR", &g_params.relaxationFactor, 0.0f, 10.0f, 0.1f);
...
int main(int argc, char* argv[])
{
	...
	// opening scene
	g_scenes.push_back(new StirredTankSmallRadius("Stirred Tank --small radius"));
	g_scenes.push_back(new StirredTank3D("Stirred Tank 3D"));
	g_scenes.push_back(new StirredTank2D("Stirred Tank 2D"));
	g_scenes.push_back(new Dambreak("Dambreak"));//dambreak
	g_scenes.push_back(new twoPhaseLiquid("2 phase liquid"));//二流体混合
	g_scenes.push_back(new RayleighTaylor3D("RayleighTaylor 3D"));//二流体混合
